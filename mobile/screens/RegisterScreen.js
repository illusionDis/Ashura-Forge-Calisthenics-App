...
export default function RegisterScreen({ navigation }) {
  ...
  const handleRegister = async () => {
    if (!username || !email || !password) { setError('Tüm alanlar zorunludur.'); return; }
    ...
    try {
      ...
      const st = err.response?.status || 'Hata';
      setApiLog({ status: `${st}`, endpoint, payload, response: err.response?.data || 'Sunucuya ulaşılamadı' });
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled">
          ...
          <View style={s.logoWrap}>
            ...
            <Text style={s.subtitle}>EFSANELERİN ARASINA KATIL</Text>
          </View>
          ...
          <View style={s.card}>
            <View style={s.inputWrap}>
              ...
              <TextInput style={s.input} placeholder="Kullanıcı Adı" placeholderTextColor="#4b5563"
            ...
            <TouchableOpacity onPress={() => { navigation.navigate('Login'); setError(''); }}>
              <Text style={s.switchText}>Zaten bir savaşçı mısın?{' '}
